...
for line in data:
    ...
    if line_breaks == 0:  # Fields part
        ...
        intervals = (
            tuple(map(int, intervals[0].split("-"))),
            tuple(map(int, intervals[1].split("-"))),
        )
...
# Part 2 ===
tickets = [
    ticket for ticket in tickets if ticket not in invalid_tickets
]  # Filter out invalid tickets
fields_candidate_positions = {
    field: set() for field in fields
}  # Candidate positions for each field
...
for i in range(ticket_size):  # For each ticket position
    for field_name, candidates in fields_candidate_positions.items():
        ...
        if (
            len(candidates) == 1
        ):  # Just one candidate - this position has no dispute and has to be assigned to them
            selected_position = candidates.pop()
            field_to_position[field_name] = selected_position
            removed.add(
                selected_position
            )  # We want to remove this position from all other fields - already assigned
            break
