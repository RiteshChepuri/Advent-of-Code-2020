                ...
                for w_ in range(w - 1, w + 2):
                    if any(
                        (
                            ...
                            z_ < 0,
                            w_ < 0,
                            (x_ == x and y_ == y and z_ == z and w_ == w),
                        )
                    ):
                        continue
        ...
        for y in range(Y):
            for x in range(X):
                ...
                if plane[z][y][x] in {
                    "#",
                    "I",
                }:  # Also consider changed in this cycle, because they all look at the same time
...
X, Y, Z, W = 26, 26, 15, 15
plane = [
    [[["." for l in range(X)] for k in range(Y)] for j in range(Z)] for i in range(W)
]
